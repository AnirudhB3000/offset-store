...
/**
 * @brief Demonstrates reading a linked list from shared memory.
 *
 * Opens an existing region, resolves the list by root name,
...
int main(int argc, char **argv)
{
    ...
    status = offset_store_get_root(&store, root_name, &list_offset);
    if (status != OFFSET_STORE_STATUS_OK) {
    ...
    length = dynlist_length(&store.region, list_offset);
    printf("list_offset=%llu length=%llu values=[",
        (unsigned long long) list_offset.offset,
        (unsigned long long) length);

    for (i = 0; i < length; i++) {
        memset(value, 0, sizeof(value));
        if (dynlist_get(&store.region, list_offset, i, value) != 0) {
            fprintf(stderr, "dynlist_get failed at index %zu\n", i);
            offset_store_close(&store);
            return 1;
        }
        printf("%s%s", value, i + 1 < length ? ", " : "");
    }
